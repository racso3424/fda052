...
// Consultar la fecha permitida de evaluación 2
$query_fecha = "SELECT evaluacion2 FROM fechas_evaluacion LIMIT 1";
$result_fecha = $conn->query($query_fecha);
$row_fecha = $result_fecha->fetch_assoc();

$fecha_permitida = $row_fecha['evaluacion2'] ?? null;
...
// Procesar el formulario para la evaluación 2
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $evaluacion_habilitada) {
    ...
    // Verificar si la matrícula ya tiene evaluación 2
    $stmt_check = $conn->prepare("SELECT * FROM evaluacion_maestro_2 WHERE matricula = ?");
    $stmt_check->bind_param("s", $matricula);
    $stmt_check->execute();
    $result_check = $stmt_check->get_result();

    if ($result_check->num_rows > 0) {
        echo "<script>alert('Esta matrícula ya tiene una evaluación registrada en la Evaluación 2.');window.location.href='maestro.php';</script>";
    } else {
        // Insertar datos
        $stmt_insert = $conn->prepare("INSERT INTO evaluacion_maestro_2 (matricula, `uno`, `dos`, `tres`, `cuatro`, `cinco`, `seis`, `siete`, promedio_2) 
                                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt_insert->execute()) {
            echo "<script>alert('Evaluación 2 registrada con éxito.'); window.location.href='maestro.php';</script>";
        } else {
            echo "<script>alert('Error al registrar la Evaluación 2.');</script>";
        }
    }
}
?>
    ...
    <div class="container py-5">
        <div class="card shadow evaluation-form">
            <div class="card-header bg-success text-white">
                <h2 class="mb-0">Registrar Evaluación 2</h2>
            </div>
            <div class="card-body">
                ...
                <form method="POST" action="" id="evaluationForm">
                    ...
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-success btn-lg" <?php echo !$evaluacion_habilitada ? 'disabled' : ''; ?>>
