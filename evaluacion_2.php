...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $evaluacion_habilitada) {
    ...
    $stmt_check = $conn->prepare("SELECT * FROM evaluacion_2 WHERE matricula = ?");
    ...
    if ($result_check->num_rows > 0) {
        ...
        // Insertar datos
        $stmt_insert = $conn->prepare("INSERT INTO evaluacion_2 (matricula, `uno`, `dos`, `tres`, `cuatro`, `cinco`, `seis`, `siete`, `ocho`, `nueve`, `diez`, promedio_2) 
                                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
