...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $evaluacion_habilitada) {
    ...
    $stmt_check = $conn->prepare("SELECT * FROM evaluacion_maestro_1 WHERE matricula = ?");
    $stmt_check->bind_param("s", $matricula);
    $stmt_check->execute();
    $result_check = $stmt_check->get_result();

    if ($result_check->num_rows > 0) {
        echo "<script>alert('Esta matrícula ya tiene una evaluación registrada.'); window.location.href='maestro.php';</script>";
    } else {
        // Insertar datos
        $stmt_insert = $conn->prepare("INSERT INTO evaluacion_maestro_1 (matricula, uno, dos, tres, cuatro, cinco, seis, siete, promedio_1) 
                                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt_insert->execute()) {
            echo "<script>alert('Evaluación registrada con éxito.'); window.location.href='maestro.php';</script>";
        } else {
    ...
    <div class="container py-5">
        <div class="card shadow evaluation-form">
            <div class="card-header bg-danger text-white">
                <h2 class="mb-0">Registrar Evaluación 1</h2>
            </div>
            <div class="card-body">
                ...
                <form method="POST" action="" id="evaluationForm">
                    ...
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-danger btn-lg" <?php echo !$evaluacion_habilitada ? 'disabled' : ''; ?>>
